...
export default function ServicesPage({ collapsed, onToggleCollapsed }) {
  ...
  const {
    ...
    hasSearchResults,
    isSearchActive
  } = useSimpleSearch('/api/services', namespace);
      ...
      <CommonTable
        columns={[
          ...
          { title: 'Type', dataIndex: 'type', render: (val, row, i, isTooltip) => {
              if (isTooltip) return val;
              // Service类型的状态标签
              const isClusterIP = val === 'ClusterIP';
              const isNodePort = val === 'NodePort';
              const isLoadBalancer = val === 'LoadBalancer';
              const isExternalName = val === 'ExternalName';
              
              let statusClass = 'status-running';
              if (isClusterIP) {
                statusClass = 'status-ready';
              } else if (isNodePort || isLoadBalancer) {
                statusClass = 'status-pending';
              } else if (isExternalName) {
          ...
          { title: 'Ports', dataIndex: 'ports', render: (val, row, i, isTooltip) => isTooltip ? val : <span>{Array.isArray(val) ? val.join(', ') : val}</span> },
        ]}
        data={displayData}
        pageSize={pageSize}
        currentPage={page}
