...
export default function NamespacesPage({ collapsed, onToggleCollapsed }) {
  ...
  const {
    ...
    hasSearchResults,
    isSearchActive
  } = useSimpleSearch('/api/namespaces', null);
      ...
      <CommonTable
        columns={[
          ...
          { title: 'Status', dataIndex: 'status', render: (val, row, i, isTooltip) => {
              ...
              // Namespace状态判断逻辑
              const isActive = val === 'Active' || val === 'Available' || val === 'Ready';
              const isTerminating = val === 'Terminating' || val === 'Deleting';
              const isPending = val === 'Pending' || val === 'Creating' || val === 'Initializing';
              const isFailed = val === 'Failed' || val === 'Error' || val === 'TerminationFailed';
              
              let statusClass = 'status-running';
              if (isActive) {
                statusClass = 'status-ready';
              } else if (isTerminating || isPending) {
