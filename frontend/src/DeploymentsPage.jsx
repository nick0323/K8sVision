...
export default function DeploymentsPage() {
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    fetch(`/api/deployments?limit=${pageSize}&offset=${(page-1)*pageSize}`)
      .then(res => res.json())
      .then(res => {
    ...
    <div>
      ...
      <CommonTable
        columns={[
          ...
          { title: 'Available', dataIndex: 'availableReplicas', render: (val, row, i, isTooltip) => isTooltip ? val : <span title={val}>{val}</span> },
          { title: 'Desired', dataIndex: 'desiredReplicas', render: (val, row, i, isTooltip) => isTooltip ? val : <span title={val}>{val}</span> },
          { title: 'Status', dataIndex: 'status', render: (val, row, i, isTooltip) => isTooltip ? val : <span className={`status-tag ${(val === 'Running' || val === 'Ready' || val === 'Healthy' || val === 'Normal') ? 'event-type-normal' : 'event-type-warning'}`} title={val}>{val}</span> },
        ]}
        data={filteredRows}
        pageSize={pageSize}
        currentPage={page}
