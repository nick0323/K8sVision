...
export default function IngressPage({ collapsed, onToggleCollapsed }) {
  ...
  const {
    ...
    hasSearchResults,
    isSearchActive
  } = useSimpleSearch('/api/ingresses', namespace);
    ...
    <div>
      ...
      <CommonTable
        columns={[
          ...
          { title: 'Class', dataIndex: 'class', render: (val, row, i, isTooltip) => isTooltip ? val : <span>{val}</span> },
          { title: 'Hosts', dataIndex: 'hosts', render: (val, row, i, isTooltip) => isTooltip ? val : <span>{Array.isArray(val) ? val.join(', ') : val}</span> },
          { title: 'Path', dataIndex: 'path', render: (val, row, i, isTooltip) => isTooltip ? val : <span>{Array.isArray(val) ? val.join(', ') : val || '-'}</span> },
          { title: 'TargetService', dataIndex: 'targetService', render: (val, row, i, isTooltip) => isTooltip ? val : <span>{Array.isArray(val) ? val.join(', ') : val}</span> },
          { title: 'State', dataIndex: 'status', render: (val, row, i, isTooltip) => {
              if (isTooltip) return val;
              
              // Ingress状态判断逻辑
              const isHealthy = val === 'Healthy' || val === 'Ready' || val === 'Available' || val === 'Active';
              const isPartial = val === 'PartialAvailable' || val === 'PartiallyAvailable' || val === 'Degraded';
              const isAbnormal = val === 'Abnormal' || val === 'Failed' || val === 'Error' || val === 'Unhealthy';
              const isPending = val === 'Pending' || val === 'Creating' || val === 'Updating' || val === 'Reconciling';
