...
export default function NodesPage({ collapsed, onToggleCollapsed }) {
  ...
  const {
    ...
    hasSearchResults,
    isSearchActive
  } = useSimpleSearch('/api/nodes', null);
    ...
    <div>
      ...
      <CommonTable
        columns={[
          ...
          { title: 'Pods', dataIndex: 'podsUsed', render: (val, row, i, isTooltip) => isTooltip ? val : <span>{val}</span> },
          { 
            title: 'State', 
            dataIndex: 'status', 
            render: (val, row, i, isTooltip) => {
              ...
              // Node状态判断逻辑
              const isReady = val === 'Ready' || val === 'Active' || val === 'Healthy';
              const isNotReady = val === 'NotReady' || val === 'Inactive';
              const isUnknown = val === 'Unknown' || val === 'Unreachable';
              const isError = val === 'Error' || val === 'Failed' || val === 'CrashLoopBackOff';
              const isPending = val === 'Pending' || val === 'Creating' || val === 'Initializing';
              
              let statusClass = 'status-running';
              if (isReady) {
                statusClass = 'status-ready';
              } else if (isNotReady || isPending) {
                statusClass = 'status-pending';
              } else if (isUnknown || isError) {
